...
MLe=function(iter){
  maxsvm0=maxnb0=maxknn0=maxlda0=maxcart0=maxRF0=matrix(0,ncol=6, nrow= Mult,byrow=T)
  max_svm=max_nb=max_knn=max_lda=max_cart=max_RF=max_ML=matrix(0,ncol=6,nrow=iter, byrow=T)
  max_svmsd=max_nbsd=max_knnsd=max_ldasd=max_cartsd=max_RFsd=max_ML=matrix(0,ncol=6,nrow=iter,byrow=T)
  ...
  for (g in 1:iter) {
    ...
    prRF=predict(fitRF,test,type = "prob")
    #confusionMatrix(prRF,test$k)
    km=c()
    m=1
    while (m<=length(test$k)) {
      km[m]=which(rmultinom(1,1,prRF[m,])==1)
      m=m+1
    }
    ...
    prRF=predict(fitRF,test12,type = "prob")
  
    km=c()
    m=1
    while (m<=length(test12$k1)) {
      km[m]=which(rmultinom(1,1,prRF[m,])==1)
      m=m+1
    }    
    ...
    prRF=predict(fitRF,test13,type = "prob")
    
    km=c()
    m=1
    while (m<=length(test13$k1)) {
      km[m]=which(rmultinom(1,1,prRF[m,])==1)
      m=m+1
    }
    ...
    prRF=predict(fitRF,test23,type = "prob")
    
    km=c()
    m=1
    while (m<=length(test23$k1)) {
      km[m]=which(rmultinom(1,1,prRF[m,])==1)
      m=m+1
    }
    ...
    init=c(rep(0.05,6))
    maxRF0[mult,]=nlminb(init,like,lower=c(0,0,0,0,0,0),upper=c(rep(Inf,6)))$par
    ...
    fitsvm <- caret::train(k~t, # Survived is a function of the variables we decided to include
                           ...
                           trControl = caret::trainControl(method = 'cv', # Use cross-validation
                                                           number = 10, classProbs = TRUE) # Use 5 folds for cross-validation
                            ) #End 
    ...
    prsvm=predict(fitsvm,test, type = "prob") 
    ...
    m=1
    while (m<=length(test$k)) {
      km[m]=which(rmultinom(1,1,prsvm[m,])==1)
      m=m+1
    }
    ...
    colnames(MM12.1)=c("t1","k1")
    train12=data.frame(MM12.1)
    train12$k1=make.names(train12$k1)
    
    fitsvm <- caret::train(k1~t1, # Survived is a function of the variables we decided to include
                           data = train12, # Use the train data frame as the training data
                           method = 'svmRadial',# Use the 'random forest' algorithm
                           trControl = caret::trainControl(method = 'cv', # Use cross-validation
                                                           number = 10 , classProbs = T) # Use 5 folds for cross-validation
    ) #End 
    ...
    prsvm=predict(fitsvm,test12,type = "prob")
    ...
    colnames(MM13.1)=c("t1","k1")
    train13=data.frame(MM13.1)
    train13$k1=make.names(train13$k1)
    ...
    prsvm=predict(fitsvm,test13,type = "prob")
    ...
    colnames(MM23.1)=c("t1","k1")
    train23=data.frame(MM23.1)
    train23$k1=make.names(train23$k1)
    ...
    prsvm=predict(fitsvm,test23,type = "prob")
    ...
    init=c(rep(0.5,6))
    maxsvm0[mult,]=nlminb(init,like,lower=c(0,0,0,0,0,0),upper=c(rep(Inf,6)))$par
    ...
    prknn=predict(fitknn,test,type = "prob")
    #confusionMatrix(prknn,test$k)
    
    km=c()
    m=1
    while (m<=length(test$k)) {
      km[m]=which(rmultinom(1,1,prknn[m,])==1)
      m=m+1
    }
    ...
    prknn=predict(fitknn,test12,type = "prob")
    ...
    prknn=predict(fitknn,test13,type = "prob")
    ...
    prknn=predict(fitknn,test23,type = "prob")
    ...
    init=c(rep(0.5,6))
    maxknn0[mult,]=nlminb(init,like,lower=c(0,0,0,0,0,0),upper=c(rep(Inf,6)))$par
    ...
    prlda=predict(fitlda,test,type = "prob")
    #confusionMatrix(prlda,test$k)
    
    km=c()
    m=1
    while (m<=length(test$k)) {
      km[m]=which(rmultinom(1,1,prlda[m,])==1)
      m=m+1
    }
    ...
    prlda=predict(fitlda,test12,type = "prob")
    ...
    prlda=predict(fitlda,test13,type = "prob")
    ...
    prlda=predict(fitlda,test23,type = "prob")
    ...
    init=c(rep(0.5,6))
    maxlda0[mult,]=nlminb(init,like,lower=c(0,0,0,0,0,0),upper=c(rep(Inf,6)))$par
    ...
    prrpart=predict(fitrpart,test,type = "prob")
    #confusionMatrix(prrpart,test$k)
    
    km=c()
    m=1
    while (m<=length(test$k)) {
      km[m]=which(rmultinom(1,1,prrpart[m,])==1)
      m=m+1
    }
    ...
    prrpart=predict(fitrpart,test12,type = "prob")
    ...
    prrpart=predict(fitrpart,test13,type = "prob")
    ...
    prrpart=predict(fitrpart,test23,type = "prob")
    ...
    init=c(rep(0.5,6))
    maxcart0[mult,]=nlminb(init,like,lower=c(0,0,0,0,0,0),upper=c(rep(Inf,6)))$par
    ...
    prnb=predict(fitnb,test,type = "prob")
    #confusionMatrix(prnb,test$k)
    
    km=c()
    m=1
    while (m<=length(test$k)) {
      km[m]=which(rmultinom(1,1,prnb[m,])==1)
      m=m+1
    }
    ...
    prnb=predict(fitnb,test12,type = "prob")
    ...
    prnb=predict(fitnb,test13,type = "prob")
    ...
    prnb=predict(fitnb,test23,type = "prob")
    ...
    init=c(rep(0.5,6))
    maxnb0[mult,]=nlminb(init,like,lower=c(0,0,0,0,0,0),upper=c(rep(Inf,6)))$par
    ...
    max_RF[g,]=apply(maxRF0,2,mean)
    max_RFsd[g,]=apply(maxRF0,2,sd)
    
    max_svm[g,]=apply(maxsvm0, 2, mean)
    max_svmsd[g,]=apply(maxsvm0, 2, sd)
    
    max_lda[g,]=apply(maxlda0, 2, mean)
    max_ldasd[g,]=apply(maxlda0, 2, sd)
    
    max_nb[g,]=apply(maxnb0, 2, mean)
    max_nbsd[g,]=apply(maxnb0, 2, sd)
    
    max_knn[g,]=apply(maxknn0, 2, mean)
    max_knnsd[g,]=apply(maxknn0, 2, sd)
    
    max_cart[g,]=apply(maxcart0, 2, mean)
    max_cartsd[g,]=apply(maxcart0, 2, sd)
  ...
  OUT=list(max_RF,max_svm,max_knn,max_lda,max_cart,max_nb,max_ML,
           max_RFsd,max_svmsd,max_knnsd,max_ldasd,max_cartsd,max_nbsd,
...
MLsd=matrix(0,nrow = 6,ncol=6)
row.names(MLsd)=c("max_RFsd","max_svmsd","max_knnsd","max_ldasd","max_cartsd","max_nbsd")
colnames(MLsd) = c("b1","b2","b3","mu1","mu2","mu3")
...
for(i in 1:6){
  MLsd[i,]=apply(results[[1]][[i+7]],2,mean)
}

for (i in 1:12) {
  percent[1,i]=mean(results[[1]][[13+i]])
}
...
print(ML-matrix(ML[8,],ncol=6,nrow=8,byrow=T))

accurm=apply(results[[1]][[26]], 2, mean, na.rm =T)
accurs=apply(results[[1]][[26]], 2, sd, na.rm =T)
